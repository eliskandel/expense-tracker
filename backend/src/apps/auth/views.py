...
# class AddUserPermissionsView(generics.GenericAPIView):
...
#             raise exceptions.APIException({"error": "Permission does not exist"})
...
class UserPermissionsView(generics.ListAPIView):
    ...
    @extend_schema(responses=PermissionsSerializer)
    def get_queryset(self):
